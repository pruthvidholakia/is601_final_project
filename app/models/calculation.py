...
class Power(Calculation):
    """
    Power: base ** exponent (exactly two inputs)
    Examples: [2, 3] -> 8; [9, 0.5] -> 3
    """
    ...
    def get_result(self) -> float:
        if not isinstance(self.inputs, list):
            raise ValueError("Inputs must be a list of numbers.")
        if len(self.inputs) != 2:
            raise ValueError("Power requires exactly two numbers: base and exponent.")
        base, exponent = self.inputs

        # Disallow 0 ** negative (division by zero)
        if base == 0 and exponent < 0:
            raise ValueError("Zero cannot be raised to a negative power.")

        # If base is negative and exponent is non-integer, Python returns a complex number.
        # We only support real-number results.
        if base < 0:
            # Treat floats that are mathematically integers as integers
            if isinstance(exponent, float) and not exponent.is_integer():
                raise ValueError("Negative base with non-integer exponent is not supported (complex result).")
